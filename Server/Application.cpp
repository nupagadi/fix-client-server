...
void Application::onMessage( const FIX42::MarketDataRequest& message, const FIX::SessionID& )
{
    ...
    const Order *topBidPtr, *topOfferPtr;
    try
    {
        topBidPtr = &m_orderMatcher.find("EURUSD", Order::buy);
        topOfferPtr = &m_orderMatcher.find("EURUSD", Order::sell);
    }
    catch(std::exception&)
    {
        std::cout << "No such a symbol" << std::endl;
    }

    FIX42::MarketDataIncrementalRefresh marketDataIncrementalRefresh;
    FIX42::MarketDataIncrementalRefresh::NoMDEntries noMDEntries;

    noMDEntries.set(FIX::MDUpdateAction('0'));
    noMDEntries.set(FIX::MDEntryType('0')); // bid
    noMDEntries.set(FIX::MDEntryPx(topBidPtr->getPrice()));
    noMDEntries.set(FIX::Symbol("EURUSD"));
    marketDataIncrementalRefresh.addGroup(noMDEntries);

//    noMDEntries.set(FIX::MDUpdateAction('0'));
    noMDEntries.set(FIX::MDEntryType('1')); // offer
    noMDEntries.set(FIX::MDEntryPx(topOfferPtr->getPrice()));
//    noMDEntries.set(FIX::Symbol("EURUSD"));
    marketDataIncrementalRefresh.addGroup(noMDEntries);
    ...
    FIX::SenderCompID senderCompID;
    FIX::TargetCompID targetCompID;
    auto& incomingHeader = message.getHeader();
    incomingHeader.getField(senderCompID);
    incomingHeader.getField(targetCompID);
    std::string target = targetCompID.getString();
    std::string sender = senderCompID.getString();

    auto& header = marketDataIncrementalRefresh.getHeader();
    // swapping for response
    header.setField( FIX::SenderCompID(target) );
    header.setField( FIX::TargetCompID(sender) );


    FIX::Session::sendToTarget( marketDataIncrementalRefresh );
